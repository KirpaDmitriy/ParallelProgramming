...
int main(int argc, char* argv[]) {
	if(argc != 3) {
		printf("Need 2 arguments: the size of the array, the number of threads\n\n");
		return 0;
	}
	unsigned int seed_i, N;
	struct timeval T1, T2;
	long delta_ms;  // , delta_ms_sort = 0;
	N = atoi(argv[1]);
	...
	for(int i=0; i<100; i++) {
		...
		#pragma omp parallel for default(none) shared(array1, N) schedule(static, 1)
		for(int mas_i=0; mas_i<N; mas_i++) {
			array1[mas_i] = func1(array1[mas_i]);
		}
		#pragma omp parallel for default(none) shared(array2, array2_copy, N) schedule(static, 1)
		for(int mas_i=0; mas_i<N/2; mas_i++) {
		...
		#pragma omp parallel for default(none) shared(array1, array2, min_array2_el, N) schedule(static, 1)
		for(int mas_i=0; mas_i<N/2; mas_i++) {
		...
		// gettimeofday(&T4, NULL);
        	// delta_ms_sort += 1000 * (T4.tv_sec - T3.tv_sec) + (T4.tv_usec - T3.tv_usec) / 1000;

		// verifying
		double X = 0;
		#pragma omp parallel for default(none) shared(N, X, array2, min_array2_el) schedule(static, 1)
		for(int mas_i=0; mas_i<N/2; mas_i++) {
	...
	delta_ms = 1000 * (T2.tv_sec - T1.tv_sec) + (T2.tv_usec - T1.tv_usec) / 1000;
        // printf("\n\nMilliseconds taken by sorting: %ld", delta_ms_sort);
